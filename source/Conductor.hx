...
class Conductor
{
	...
	public static var safeZoneOffset:Float = (ClientPrefs.safeFrames / 60) * 2000; // is calculated in create(), is safeFrames in milliseconds (* 1000),* 2000
	#else
	public static var safeZoneOffset:Float = (ClientPrefs.safeFrames / 60) * 1000;
	#end*/

	public static var safeZoneOffset:Float = (ClientPrefs.safeFrames / 60) * 3000;//1000
	public static var timeScale:Float = Conductor.safeZoneOffset / 500;//166
	...
	public static function judgeNote(note:Note, diff:Float=0) //STOLEN FROM KADE ENGINE (bbpanzu) - I had to rewrite it later anyway after i added the custom hit windows lmao (Shadow Mario)
	{
		...
		#if EXTEND_JUDGES_ALLOWED
		var timingWindows:Array<Int> = [ClientPrefs.marvelousWindow, ClientPrefs.fantasticWindow, ClientPrefs.epicWindow, ClientPrefs.sickWindow, ClientPrefs.coolWindow, ClientPrefs.goodWindow, ClientPrefs.badWindow];
		var windowNames:Array<String> = ['marvelous', 'fantastic', 'epic', 'sick', 'cool', 'good', 'bad'];//I will re add that.
		#else
		...
		var windowNames:Array<String> = ['marvelous', 'sick', 'good', 'bad'];//I will re add that.
		#end
	...
	/*public static function judgeNote(note:Note, diff:Float=0):Rating // die
	{
		...
		return data[data.length - 1];//they are begin mad
	}*/
...
class Rating
{
	...
	//public var ratingMod:Float = 1.001;
	//public var score:Int = 850;
