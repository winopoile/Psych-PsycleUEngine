...
class NoteOffsetState extends MusicBeatState
{
	...
	var newCoolText:FlxText;
	var rating:FlxSprite;
	var timing:FlxSprite;
	...
	override public function create()
	{
		...
		newCoolText = new FlxText(0, 0, 0, '', 32);
		...
		timing = new FlxSprite().loadGraphic(Paths.image('early'));
		...
		{
			var numScore:FlxSprite = new FlxSprite(53 * daLoop).loadGraphic(Paths.image('num' + i));//43
	...
	var startStatusOffset:FlxPoint = new FlxPoint();
	var startMousePos2:FlxPoint = new FlxPoint();
	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
            ...
            var controlArrayTwo:Array<Bool> = [
			...
			if(controlArrayTwo.contains(true))
			{
				for (i in 0...controlArrayTwo.length)
				{
					if(controlArrayTwo[i])
					{
						switch(i)
						{
							case 0:
								ClientPrefs.statusOffset[0] -= addNum;
							case 1:
								ClientPrefs.statusOffset[0] += addNum;
							case 2:
								ClientPrefs.statusOffset[1] += addNum;
							case 3:
								ClientPrefs.statusOffset[1] -= addNum;
							case 4:
								ClientPrefs.statusOffset[2] -= addNum;
							case 5:
								ClientPrefs.statusOffset[2] += addNum;
							case 6:
								ClientPrefs.statusOffset[3] += addNum;
							case 7:
								ClientPrefs.statusOffset[3] -= addNum;
						}
					}
			...
			if(FlxG.mouse.justPressed) {
				holdingObjectType = null;
				if(ClientPrefs.ratesOnHud) {
				FlxG.mouse.getScreenPosition(camHUD, startMousePos2);
				}
				FlxG.mouse.getScreenPosition(camGame, startMousePos2);
                if (startMousePos2.x - timing.x >= 0 && startMousePos2.x - timing.x <= timing.width &&
				        startMousePos2.y - timing.y >= 0 && startMousePos2.y - timing.y <= timing.height)
				{
					holdingObjectType = false;
					startStatusOffset.x = ClientPrefs.statusOffset[0];
					startStatusOffset.y = ClientPrefs.statusOffset[1];
				}
				else if (startMousePos2.x - comboSpr.x >= 0 && startMousePos2.x - comboSpr.x <= timing.width &&
				        startMousePos2.y - comboSpr.y >= 0 && startMousePos2.y - comboSpr.y <= timing.height)
				{
					holdingObjectType = false;
					startStatusOffset.x = ClientPrefs.statusOffset[2];
					startStatusOffset.y = ClientPrefs.statusOffset[3];
				}
			}
			...
			if(holdingObjectType != null)
			{
				...
				if(ClientPrefs.ratesOnHud) {
					...
					var mousePos2:FlxPoint = FlxG.mouse.getScreenPosition(camHUD);
				}
					var mousePos:FlxPoint = FlxG.mouse.getScreenPosition(camGame);
					var mousePos2:FlxPoint = FlxG.mouse.getScreenPosition(camGame);
					var addNum:Int = holdingObjectType ? 4 : 0;
					...
					ClientPrefs.statusOffset[addNum + 0] = Math.round((mousePos2.x - startMousePos2.x) + startStatusOffset.x);
					ClientPrefs.statusOffset[addNum + 1] = -Math.round((mousePos2.y - startMousePos2.y) - startStatusOffset.y);
					repositionCombo();
				}
			}
			...
			if(controls.RESET)
			{
				...
				for (i in 0...ClientPrefs.statusOffset.length)
				{
					ClientPrefs.statusOffset[i] = 0;
				}
				repositionCombo();
			}
		}
	...
	function repositionCombo()
	{
        ...
        timing.screenCenter();
		timing.x = newCoolText.x + 70 + ClientPrefs.statusOffset[0];
		timing.y -= 35 + ClientPrefs.statusOffset[1];

		comboSpr.screenCenter();
		comboSpr.x = xThing + 50 + newCoolText.x + ClientPrefs.statusOffset[2];
		comboSpr.y -= -60 + ClientPrefs.statusOffset[3];
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				...
				case 4: dumbTexts.members[i].text = 'Timing Sprite Offset:';
				case 5: dumbTexts.members[i].text = '[' + ClientPrefs.statusOffset[0] + ', ' + ClientPrefs.statusOffset[1] + ']';
				case 6: dumbTexts.members[i].text = 'Combo Sprite Offset';
				case 7: dumbTexts.members[i].text = '[' + ClientPrefs.statusOffset[2] + ', ' + ClientPrefs.statusOffset[3] + ']';
			}
		}
	}
