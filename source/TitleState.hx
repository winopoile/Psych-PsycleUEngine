...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		}//Reworked Music Change

		Conductor.changeBPM(titleJSON.bpm);
		persistentUpdate = true;

		if(ClientPrefs.MenuMusics == 'FreakyMenu')
        Conductor.changeBPM(titleJSON.bpm = 102);//Fixed From Main
		else if(ClientPrefs.MenuMusics == 'FunkyMenu')
        Conductor.changeBPM(titleJSON.bpm = 140);//Also You Can Do This.
        /*else
		Conductor.changeBPM(titleJSON.bpm);*/
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxTween.tween(logoBl, {x: 1500}, 1.4, {ease: FlxEase.expoInOut});//FlxEase.expoInOut
				FlxTween.tween(gfDance, {x: 1500}, 1.4, {ease: FlxEase.expoInOut});
	...
	override function beatHit()
	{
		...
		if(zooming && curBeat % 4 == 0)//2
		{
			FlxG.camera.zoom = 1.1;//Changed Cuz BPM
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				createCoolText(['+ Psycle Engine by'], 15);
				#end
				case 16:
				#if PSYCLE_WATERMARKS
				addMoreText('Psycle', 30);
				//addMoreText('Soon', 15);
				#end
				case 17:
					deleteCoolText();
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
			   ...
			   {
				if (logoBl.angle == -5)//-10
					FlxTween.angle(logoBl, logoBl.angle, 5, 5.0, {ease: FlxEase.quartInOut});
				if (logoBl.angle == 5)//10
					FlxTween.angle(logoBl, logoBl.angle, -5, 5.0, {ease: FlxEase.quartInOut});
			   }, 0);

			    FlxTween.tween(logoBl, {x: 150}, 1.5, {ease: FlxEase.elasticInOut});//FlxEase.expoInOut, Default 1.4
				//FlxTween.tween(logoBl, {y: 150}, 1.4, {ease: FlxEase.expoInOut});
